...
const App = () => {
  ...
  const handleRestore= async (todo) => {
    try {
      // Add to accomplished collection
      const response=await axios.post('http://localhost:5050/todos', { title: todo.title,completed:false });
      // Delete from todos collection
      await axios.delete(`http://localhost:5050/accomplished/${todo._id}`);
      setTodos([...todos,response.data])
      setAccomplishedTasks(accomplishedTasks.filter(task => task._id !== todo._id));
  ...
  return (
    <div className="App">
      ...
      <NewTodoList todos={accomplishedTasks} onDelete={handleDeleteAccomplished} onRestore= {handleRestore}/>
    </div>
  );
};
